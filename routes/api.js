...
router.get('/search', function (req, res, next) {
  commonmethods.getReceipeList(function (err, data) {
    if (err)
      res.json({ status: 'ERROR', error: err });
    else {
      res.render('searchreceipe', { menuitems: data });
    }
  });
});

/* GET Filtered receipe list. */
router.get('/filteredreceipe', function (req, res, next) {  
  var searchcontent = req.query.id;  
  commonmethods.getFilteredReceipe(searchcontent, function (err, data) {
    if (err)
      res.json({ status: 'ERROR', error: err });
    else {
      res.render('receipe', { menuitems: data });
    }
  });
});

/* GET  menu  */
router.get('/menusearch', function (req, res, next) {
  commonmethods.getMenuList(function (err, data) {
    if (err)
      res.json({ status: 'ERROR', error: err });
    else {
      res.render('searchmenu', { menuitems: data });
    }
  });
});

/* GET Filtered receipe list. */
router.get('/filteredmenu', function (req, res, next) { 
  var searchcontent = req.query.id;  
  commonmethods.getFilteredMenu(searchcontent, function (err, data) {
    if (err)
      res.json({ status: 'ERROR', error: err });
    else {
